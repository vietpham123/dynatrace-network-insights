...
export const NETWORK_RCA_WORKFLOW: any = {
  ...
  "tasks": {
    ...
    "edges": {
      ...
      "conditions": {},
      "input": {
        "query": "timeseries e=count(cno.dep.uses), by:{`device.name`, `device.address`, `upstream.name`, `upstream.address`, link_type}, from:-2h | fieldsAdd n=arraySum(e) | filter n>0 | fields downstream=if(isNotNull(`device.name`) and `device.name` != \"n/a\" and `device.name` != \"\", lower(`device.name`), else: `device.address`), upstream=if(isNotNull(`upstream.name`) and `upstream.name` != \"n/a\" and `upstream.name` != \"\", lower(`upstream.name`), else: `upstream.address`), link_type"
      },
      "name": "edges",
      "position": {
        "x": 1,
