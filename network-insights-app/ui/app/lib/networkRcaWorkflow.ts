// Clean, importable unified network-RCA workflow — one classify pass routes each device to device/chain
// RCA (not reporting SNMP -> graph root + suppress downstream) or interface RCA (reporting but admin-enabled
   ...
   collapses every such device onto one join key and RCA blames the wrong box. */
{
  ...
  "tasks": {
    "reach": {
      ...
      "conditions": {},
      "input": {
        "query": "timeseries s=count(cno.if.oper_status), by:{sys_name}, from:-2m | fieldsAdd device=if(isNotNull(sys_name) and sys_name != \"n/a\" and sys_name != \"\", lower(sys_name), else: `device.address`), is_up=if(arraySum(s)>0,1,else:0) | fields device, is_up"
      },
      "name": "reach",
      "position": {
        "x": 0,
    ...
    "edges": {
      ...
      "conditions": {},
      "input": {
        "query": "timeseries e=count(cno.dep.uses), by:{`device.name`, `upstream.name`, link_type}, from:-2h | fieldsAdd n=arraySum(e) | filter n>0 | fields downstream=lower(`device.name`), upstream=lower(`upstream.name`), link_type"
      },
      "name": "edges",
      "position": {
        "x": 1,
    ...
    "entities": {
      ...
      "conditions": {},
      "input": {
        "query": "timeseries s=count(cno.if.oper_status), by:{sys_name, `dt.entity.network:device`}, from:-2h | fieldsAdd n=arraySum(s) | filter n>0 | fields device=if(isNotNull(sys_name) and sys_name != \"n/a\" and sys_name != \"\", lower(sys_name), else: `device.address`), entityId=`dt.entity.network:device` | dedup device"
      },
      "name": "entities",
      "position": {
        "x": 2,
    ...
    "classify": {
      ...
      "conditions": {},
      "input": {
        "query": "timeseries oper = avg(cno.if.oper_status), admin = avg(cno.if.admin_status), by:{sys_name, if_descr}, from:-5m | fieldsAdd o = arrayLast(oper), a = arrayLast(admin), device = if(isNotNull(sys_name) and sys_name != \"n/a\" and sys_name != \"\", lower(sys_name), else: `device.address`) | filter isNotNull(o) and a == 1 | summarize down_list = collectArray(if(o==2, if_descr)), up = countIf(o==1), down = countIf(o==2), by:{device} | fieldsAdd down_list = arrayRemoveNulls(down_list) | filter up > 0 and down > 0 | sort down desc"
      },
      "name": "classify",
      "position": {
        "x": 3,
