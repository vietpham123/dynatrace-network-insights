#!/usr/bin/env node
/*
 * Generates the standalone NetFlow → Dynatrace collector configs (full / aggregated / sampled)
 * into ~/Desktop/netflow-collector-configs/. The base mirrors the production Bindplane-managed
 * cno-netflow pipeline (bearertokenauth + persistent sending_queue + retry + drop-raw transform +
 * grpc/http); each mode layers on only its one processor. Env-var placeholders — never a token.
 *
 * KEEP IN SYNC with the app generator:
 *   network-insights-app/ui/app/lib/netflowCollectorConfig.ts   (same template + logic)
 *
 * Run:  node scripts/gen_netflow_configs.js
 */
const fs = require("fs");
const os = require("os");
...
const LABEL = { full: "FULL (every flow)", sampled: "SAMPLED (1-in-N)" };
const INTRO = {
  full: () =>
...
const OUT = path.join(os.homedir(), "Desktop", "netflow-collector-configs");
fs.mkdirSync(OUT, { recursive: true });
for (const mode of ["full", "sampled"]) {
  const f = path.join(OUT, `otel-collector-${mode}.yaml`);
  fs.writeFileSync(f, generate(mode, 100));
  console.log(`wrote ${f}`);
}
